...
#endif
{
	...
	if (inode) {
			...
			likely(susfs_is_current_proc_umounted()))
		{
			...
			if (!dpath) { kfree(pathname); goto inotify_orig; }
			if (kern_path(dpath, 0, &path)) { kfree(pathname); goto inotify_orig; }
			if (!path.dentry->d_inode) { path_put(&path); kfree(pathname); goto inotify_orig; }
